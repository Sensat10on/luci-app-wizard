...
После первого ручного `apk add --allow-untrusted` можно добавить каталог релиза как источник пакета:

```sh
echo 'https://github.com/Sensat10on/luci-app-wizard/releases/download/v0.1.0/' > /etc/apk/repositories.d/luci-app-wizard.list
apk update --allow-untrusted
```

Этот предварительный личный репозиторий пока не подписан. Перед обновлением сверяйте ссылку на релиз и контрольную сумму; поддержка подписанного каталога будет добавлена в следующем выпуске.
